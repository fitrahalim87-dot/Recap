...
import { Upload, X, Loader2, Sparkles, Copy, Trash2, FileText, History as HistoryIcon, Hash, BookOpen, Clock, ChevronDown, ChevronUp, Maximize2, Minimize2 } from 'lucide-react';
...
export default function App() {
  ...
  const toggleFullscreen = () => {
    if (!document.fullscreenElement) {
      document.documentElement.requestFullscreen().catch((err) => {
        console.warn(`Fullscreen API blocked: ${err.message}. Enabling layout fallback.`);
      });
  ...
  const handleGenerateBatch = async (batchId: string) => {
    ...
    try {
      const imageData = await Promise.all(batch.images.map(async img => {
  ...
  return (
    <div className={`bg-zinc-50 p-4 md:p-8 space-y-12 pb-32 transition-all duration-300 ${isFullscreen ? 'fixed inset-0 z-[9999] overflow-y-auto max-w-none w-full h-screen scrollbar-thin' : 'min-h-screen max-w-5xl mx-auto'}`}>
      {/* Header */}
      <header className="flex flex-col md:flex-row justify-between items-center gap-8 py-6">
        ...
        <div className="flex items-center gap-3 flex-wrap justify-center">
          <button 
            ...
            className={`flex items-center gap-2 font-sans font-semibold text-sm py-2.5 px-5 rounded-xl transition-all duration-300 shadow-sm
              ${isFullscreen 
          ...
          <button 
            ...
            className={`flex items-center gap-2 font-sans font-semibold text-sm py-2.5 px-5 rounded-xl transition-all duration-350
              ${showHistory 
